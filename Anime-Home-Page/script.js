...
// RANDOM SERIE - - - - RANDOM SERIE

document.addEventListener("DOMContentLoaded", function() {
    var randomSeriesWrappers = document.querySelectorAll('.random-series-wrp'); // Selectăm toate wrapper-ele pentru serii aleatoare
    ...
    randomSeriesWrappers.forEach(function(wrapper) { // Iterăm prin fiecare wrapper
        ...
        function showRandomSerie() {
            // Ascunde toate seriile din wrapper-ul curent
            randomSeries.forEach(function(serie) {
                serie.style.display = 'none';
            });

            // Afișează seria aleatorie din wrapper-ul curent
            randomSeries[currentIndex].style.display = 'block';
        }

        // Verifică dacă a trecut intervalul de 5 secunde de la ultima actualizare
        var currentTime = new Date().getTime();
        var difference = currentTime - lastUpdateTime;
        if (difference >= interval) {
            // Generează un index aleatoriu dacă a trecut intervalul de 5 secunde
            currentIndex = Math.floor(Math.random() * randomSeries.length);
            localStorage.setItem('randomSeriesIndex', currentIndex);
            localStorage.setItem('lastUpdateTime', currentTime);
        }

        // Afișează o serie la început pentru fiecare wrapper
        showRandomSerie();

        // Afișează o serie aleatorie la fiecare 5 secunde pentru fiecare wrapper
        setInterval(function() {
            // Generează un index aleatoriu
            currentIndex = Math.floor(Math.random() * randomSeries.length);
            localStorage.setItem('randomSeriesIndex', currentIndex);
